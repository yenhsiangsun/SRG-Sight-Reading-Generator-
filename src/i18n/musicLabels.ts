...
import {instrumentTranslations} from './instrumentTranslations';
...
export function instrumentLabel(id:Instrument,locale:Locale) {
  ...
  if (named) return named.instruments[id];
  if(locale==='zh-TW')return INSTRUMENTS[id].zh;
  const index = ['ja','es','de','fr','ko','pt-BR','ru','it'].indexOf(locale);
  if (index >= 0 && instrumentTranslations[id]?.[index]) return instrumentTranslations[id][index];
...
const scaleNames:Record<string,[string,string]>={
  'egyptian-pentatonic':['Egyptian pentatonic','エジプト五音音階'],
  'pelog-pentatonic':['Pelog pentatonic (12-TET approximation)','ペロッグ五音音階（十二平均律近似）'],
  'slendro-pentatonic':['Slendro pentatonic (12-TET approximation)','スレンドロ五音音階（十二平均律近似）'],
...
export function scaleLabel(id:string,original:string,locale:Locale){
  const named = namedCatalogs[locale];
  if (named) return named.scales[id] ?? original;
  return locale==='zh-TW'?original:scaleNames[id]?.[locale==='ja'?1:0]??id;
}
